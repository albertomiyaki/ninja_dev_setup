...
$sync.ChromeProfilePath = "$env:LOCALAPPDATA\Google\Chrome\User Data\Default"
$sync.SelectedBackupOptions = @()
$sync.SelectedRestoreOptions = @()
$sync.BackupItems = @{
    "Favorites" = @{
        Path = "Bookmarks"
        Description = "Chrome bookmarks and favorites"
    }
    "SearchEngines" = @{
        Path = "Web Data"
...
<Window 
    ...
    <Grid>
        ...
        <Border Grid.Row="0" Background="#2d2d30" Padding="15">
            <Grid>
                <StackPanel>
                    <TextBlock Text="Chrome Settings Manager" FontSize="22" Foreground="White" FontWeight="Bold"/>
                    <TextBlock Text="Backup and restore your Google Chrome browser settings" Foreground="#999999" Margin="0,5,0,0"/>
                </StackPanel>
            </Grid>
        </Border>
...
function New-ChromeBackup {
    ...
    try {
        ...
        $tempDir = Join-Path $env:TEMP "ChromeBackup_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
        ...
        foreach ($option in $Options) {
            ...
            $sourcePath = Join-Path $sync.ChromeProfilePath $item.Path
        ...
        $metadata = @{
            ...
            Options = $Options
            ChromeVersion = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe" -ErrorAction SilentlyContinue).Version
        }
        ...
        Write-ActivityLog "Error creating backup: $($_ -replace ':', '')" -Type Error
        [System.Windows.MessageBox]::Show(
...
function Initialize-RestoreOptions {
    ...
    try {
        ...
        $tempDir = Join-Path $env:TEMP "ChromeRestore_Temp"
        ...
        $infoBlock.Text = "Backup created on: $($metadata.CreatedOn)`nChrome version: $($metadata.ChromeVersion)"
        ...
        Write-ActivityLog "Error loading restore options: $($_ -replace ':', '')" -Type Error
        [System.Windows.MessageBox]::Show(
    ...
    try {
        ...
        if ($chromeProcess) {
            $result = [System.Windows.MessageBox]::Show(
                "Google Chrome is currently running. It must be closed before restoring settings.`n`nWould you like to close Chrome now?",
                "Close Chrome",
            ...
            if ($result -eq [System.Windows.MessageBoxResult]::Yes) {
                Write-ActivityLog "Closing Google Chrome" -Type Information
                $chromeProcess | ForEach-Object { $_.CloseMainWindow() | Out-Null }
                Start-Sleep -Seconds 2
                
                $chromeProcess = Get-Process -Name "chrome" -ErrorAction SilentlyContinue
                if ($chromeProcess) {
                    Write-ActivityLog "Forcing Chrome to close" -Type Warning
                    $chromeProcess | Stop-Process -Force
                    # Allow time for system to release file locks
                    Start-Sleep -Seconds 5
                }
            } else {
                Write-ActivityLog "Restore cancelled - Chrome must be closed first" -Type Warning
                return
            }
        }
        
        # Add additional check for chrome.exe processes that might be running in background
        $chromeProcesses = Get-Process | Where-Object { $_.Name -like "*chrome*" } | Where-Object { $_.Name -ne "ChromeSettingsManager" }
        if ($chromeProcesses) {
            Write-ActivityLog "Found additional Chrome-related processes, attempting to close them" -Type Warning
            $chromeProcesses | Stop-Process -Force -ErrorAction SilentlyContinue
            Start-Sleep -Seconds 3
        }
        
        # Extract files from the backup
        $tempDir = Join-Path $env:TEMP "ChromeRestore_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
        New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
        ...
        foreach ($option in $Options) {
            ...
            $destPath = Join-Path $sync.ChromeProfilePath $item.Path
                    ...
                    if ($item.IsFolder -eq $true) {
                        ...
                        try {
                            ...
                            $action = New-ScheduledTaskAction -Execute "PowerShell.exe" -Argument "-NoProfile -WindowStyle Hidden -Command `"
                                ...
                                Unregister-ScheduledTask -TaskName 'ChromeRestore_FileCopy' -Confirm:`$false
                            `""
                            ...
                            $task = Register-ScheduledTask -TaskName "ChromeRestore_FileCopy" -Action $action -Trigger $trigger -Principal $principal -Settings $settings -Force
        ...
        [System.Windows.MessageBox]::Show(
            "Restore completed successfully!`n`nYou may need to restart Google Chrome for all changes to take effect.",
            "Restore Complete",
            [System.Windows.MessageBoxButton]::OK,
            [System.Windows.MessageBoxImage]::Information
        )
    } catch {
        Write-ActivityLog "Error restoring settings: $($_ -replace ':', '')" -Type Error
        [System.Windows.MessageBox]::Show(
...
$sync.BackupButton.Add_Click({
    ...
    $saveFileDialog.Title = "Save Chrome Settings Backup"
    $saveFileDialog.FileName = "ChromeBackup_$(Get-Date -Format 'yyyyMMdd').zip"
    ...
    if ($saveFileDialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        New-ChromeBackup -BackupPath $saveFileDialog.FileName -Options $sync.SelectedBackupOptions
    }
})
...
$sync.BrowseRestoreButton.Add_Click({
    ...
    $openFileDialog.Title = "Select Chrome Settings Backup"
...
$sync.RestoreButton.Add_Click({
    ...
    $result = [System.Windows.MessageBox]::Show(
        "This will replace your current Chrome settings with the ones from the backup.`n`nDo you want to continue?",
        "Confirm Restore",
        [System.Windows.MessageBoxButton]::YesNo,
        [System.Windows.MessageBoxImage]::Warning
    )
    ...
    if ($result -eq [System.Windows.MessageBoxResult]::Yes) {
        ...
        Restore-ChromeSettings -BackupPath $sync.RestoreFilePathTextBox.Text -Options $sync.SelectedRestoreOptions
    }
})
...
Write-ActivityLog "Chrome Settings Manager started." -Type Information
